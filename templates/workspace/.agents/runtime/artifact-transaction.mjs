...
export async function executeArtifactTransaction({
  stageId,
  allowedArtifacts,
  prepareOutputs,
  ...
  try {
    ...
    if (!operationId || !artifactId || !allowedArtifacts?.includes(artifactId)) {
      fail('AIH_CONTRACT_INVALID', '当前领域动作不允许写入产物：' + artifactId);
    }
    ...
    if (!definition || !['internal-model', 'internal-model-set'].includes(definition.authorityKind)) {
    ...
    let members = collection
      ? await readCandidateSet(inputArgument, paths, definition.format)
      : [{ actor: null, data: parseCandidate(await readCandidate(inputArgument), definition.format) }];
    if (prepareCandidate) {
    ...
    const writes = [
      ...members.map((member) => ({
        ...
        content: stringifyStructured(member.data, definition.format),
      })),
