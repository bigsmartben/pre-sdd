...
示例：Baseline 的 `FDBI-004` 要求结算页达到 `USER_PATH`，并引用 `TC-007`；编译器会生成稳定的 `VSI-FDBI-004-PAGE-01`，不会查询 Figma 或决定 Flutter Widget。
...
- Provider（提供方）或实现字段，例如 Figma node、Flutter Widget、Dart Class、Finder、Mock fixture，都会被 Schema 拒绝。
- 旧视觉载体只返回 `LEGACY_VISUAL_WORKFLOW_FORBIDDEN`；不转换、不别名、不回退。

## 边界

本 Skill 不读取 Figma、不创建 Flutter Widget、不生成 Preview，也不判断主观视觉是否一致。
