...
5. Technical Validation 只从已验证 Use Cases 与 System Boundary 提取标记为需要技术验证的关键能力。每个关键能力必须映射到已选择的技术方案，以及与该选择一致、源代码哈希匹配的真实代码测试通过结论；不得要求覆盖全部架构能力，也不得把技术验证证据伪装成产品事实。
6. 对全部实际变更路径重新调用 Resolver，并按 Manifest 返回顺序执行所有验证命令；Skill 不维护静态命令清单，也不自行判断 readiness。
7. 只有用户请求移交且 Manifest 存在合法移交边时，执行登记的 handoff operation；不得保存用户审批或自动初始化消费者。
