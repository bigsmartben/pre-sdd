...
try {
  ...
  const currentRevision = argument('source-revision');
  const currentDigest = argument('source-digest');
  if (
  ...
  if (Boolean(currentRevision) !== Boolean(currentDigest)) {
    blockers.push({ code: 'FGC_SOURCE_INVALID', message: 'source-revision 与 source-digest 必须同时提供。' });
  } else if (currentDigest && !/^sha256:[a-f0-9]{64}$/.test(currentDigest)) {
    blockers.push({ code: 'FGC_SOURCE_INVALID', message: 'source-digest 格式无效。' });
  } else if (
    currentRevision
    && (coverage.source?.revision !== currentRevision || coverage.source?.digest !== currentDigest)
  ) {
    for (const item of coverage.items ?? []) staleItems.add(item.itemId);
  }
  if (
  ...
  if (coverage.source?.scope?.kind === 'node') {
    for (const item of coverage.items ?? []) {
      if ((item.anchors ?? []).some((anchor) => !coverage.source.scope.refs.includes(anchor.nodeId))) {
