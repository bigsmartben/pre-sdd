...
- Review Case：`mockCases` 直接引用 Route、Screen、Scenario、Mock Behavior 和合法 State Matrix Entry；普通 Review 地址中的 `mockcase-switcher` 只通过 `psp:mockcase-request` / `psp:mockcase-ready` 驱动真实 Lit/MSW 状态。`psp-case` 可深链，`annotate=0` 与 `mockcase=0` 独立隐藏各自工具
