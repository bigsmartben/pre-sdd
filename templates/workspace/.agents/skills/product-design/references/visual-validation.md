...
- 使用场景声明的事件实际点击、输入或选择；Wireflow 状态暴露对应 `data-state-id`，组件局部状态暴露 `data-component-state`。
- 每个本地资源由 `assets[].path` 指向真实文件，并在界面中实际使用。
...
7. 运行自动无障碍规则，并检查键盘可达、可见焦点、可访问名称和每个声明 Control 至少 24×24 CSS 像素。
