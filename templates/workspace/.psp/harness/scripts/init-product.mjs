...
const operationId = 'initialize-product';
let result;
...
async function rollback(createdFiles, stageRoot, stageRootExisted) {
  for (const path of [...createdFiles].reverse()) {
    await rm(repositoryFile(root, path), { force: true });
  }
  if (!stageRootExisted) {
    await rm(repositoryFile(root, stageRoot), { recursive: true, force: true });
  }
}
...
try {
  ...
  if (dryRun) {
    ...
    try {
      ...
      const validation = spawnSync(process.execPath, [resolve(import.meta.dirname, 'validate-product.mjs'), '--json'], {
        cwd: root,
        encoding: 'utf8',
        env: {
          ...process.env,
      ...
      if (validation.status !== 0) {
        ...
        throw Object.assign(new Error('初始化结构校验失败：' + evidence), {
...
if (json) console.log(JSON.stringify(result, null, 2));
  ...
  console.log('[PASS] ' + (dryRun ? '产品初始化预检通过。' : '产品 Package 已原子初始化。'));
