...
try {
  ...
  if (!stage || stage.status !== 'active') {
    ...
    if (!uiSpec.htmlMocks?.length && uiSpec.metadata?.status === 'draft') {
      ...
      const vitePath = process.env.PRE_SDD_VITE_BIN || repositoryFile(root, 'node_modules/vite/bin/vite.js');
      const viteConfigArguments = process.env.PRE_SDD_VITE_CONFIG
        ? ['--config', process.env.PRE_SDD_VITE_CONFIG]
        : [];
      let serverErrors = '';
      serverProcess = spawn(process.execPath, [vitePath, 'preview', ...viteConfigArguments, '--host', '127.0.0.1', '--port', String(port)], {
        cwd: areaPath,
        windowsHide: true,
      ...
      try {
        ...
        const detail = serverErrors.trim();
