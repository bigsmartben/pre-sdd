...
export function resolveHarness(manifest, project, inputPaths, intent, root) {
  ...
  for (const path of normalizedInputs) {
    ...
    const effectiveMatches = workspaceMatches.length > 0 ? workspaceMatches : matches;
  ...
  for (const scope of selected) {
    ...
    if (
      intent === 'readiness'
      && !['static', 'workspace'].includes(scope.selector.type)
  ...
  function scopeStage(scope) {
    return ['static', 'workspace'].includes(scope?.selector?.type) ? null : scope?.selector?.stage;
  }
  ...
  for (const scope of selected) {
    ...
    const stageState = ['static', 'workspace'].includes(scope.selector.type)
      ? null
