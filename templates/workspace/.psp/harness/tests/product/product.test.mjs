...
function runAreaCommand(root, script) {
  ...
  return spawnSync(process.execPath, [
    resolve(runtimeRoot, '.psp/harness/scripts/run-project-command.mjs'),
    '--area',
    'html-mock',
    '--script',
    script,
...
test('dry-run is non-mutating and initialization materializes the complete bound Package', async () => {
  ...
  assert.equal(await pathExists(stagePath), true);
...
test('collision and failed validation preserve the uninitialized state', async () => {
  ...
  assert.equal(await pathExists(resolve(rollbackRoot, rollbackProject.stages['product-design'].root)), true);
});
