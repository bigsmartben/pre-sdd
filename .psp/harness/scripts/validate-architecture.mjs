...
if (project && manifest && stage?.status === 'uninitialized' && !initializing) {
  let partial = false;
  try {
    partial = await directoryHasFiles(stage.root);
  } catch (error) {
...
if (project && manifest && blockers.length === 0) {
  const upstreamValidation = spawnSync(
    process.execPath,
    [resolve(import.meta.dirname, 'validate-product.mjs'), '--strict', '--json'],
    {
      cwd: root,
      encoding: 'utf8',
      env: {
  ...
  if (upstreamValidation.status !== 0) {
    const parsed = parseValidatorOutput(upstreamValidation);
    const codes = [...new Set((parsed?.blockers || []).map((item) => item.code))];
    block(
      'AIH_UPSTREAM_NOT_READY',
      '产品 strict Profile 未通过' + (codes.length ? '：' + codes.join(', ') : '。'),
      'product-design',
    );
  } else {
    try {
      const upstreamPaths = artifactPaths(project, 'capabilities', 'product-design');
      capabilities = parseYaml(await readFile(repositoryFile(root, upstreamPaths.internalModel), 'utf8'));
    } catch (error) {
...
if (project && manifest && blockers.every((item) => item.code !== 'AIH_UPSTREAM_NOT_READY')) {
  ...
  for (const registry of manifest.artifactRegistry.filter((item) => item.stage === stageId)) {
    ...
    try {
      const model = parseYaml(await readFile(repositoryFile(root, paths.internalModel), 'utf8'));
...
if (capabilities && blockers.every((item) => item.code !== 'AIH_ARTIFACT_SCHEMA_FAILED')) {
  ...
  for (const decision of technicalValidation?.decisions || []) {
    ...
    for (const object of [...decision.inputModels, ...decision.outputModels]) {
      requireReference(object, objectIds, 'technical-validation.decisions.' + decision.id + '.models');
      const model = objectById.get(object);
      if (model && !decision.capabilities.some((capability) => model.capabilities.includes(capability))) {
        block('AIH_REFERENCE_UNRESOLVED', decision.id + ' 的对象模型未映射到所选 capability：' + object, 'technical-validation.decisions.' + decision.id);
      }
    }
  ...
  for (const experiment of technicalValidation?.experiments || []) {
    ...
    if (runnerAvailable) {
      ...
      let description;
      try {
        description = JSON.parse(described.stdout);
      } catch {
        // Report the stable runner contract blocker below.
      }
  ...
  if (strict) {
    ...
    for (const capability of architectureCapabilities) {
      ...
      if (capability.selectionRequired && (!capability.inputs.length || !capability.outputs.length)) block('AIH_ARTIFACT_INCOMPLETE', '需要技术选型的能力必须同时定义输入和输出：' + capability.id, 'system-boundary.capabilities.' + capability.id);
    }
    for (const constraint of systemBoundary.constraints) {
    ...
    const requiredCapabilities = new Set(architectureCapabilities.filter((item) => item.selectionRequired).map((item) => item.id));
    const selectedCapabilities = unionOf(technicalValidation.decisions, 'capabilities');
    if (!sameSet(requiredCapabilities, selectedCapabilities)) block('AIH_REFERENCE_UNRESOLVED', '技术决策必须精确覆盖所有 selectionRequired capability。', 'technical-validation.decisions');
    for (const decision of technicalValidation.decisions) {
      ...
      if (!decision.inputModels.length || !decision.outputModels.length) block('AIH_ARTIFACT_INCOMPLETE', '技术决策必须绑定输入和输出对象模型：' + decision.id, 'technical-validation.decisions.' + decision.id);
