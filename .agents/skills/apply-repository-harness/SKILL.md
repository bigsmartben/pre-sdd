...
5. When the bound stage is `uninitialized`, do not create user artifacts unless the user explicitly starts that stage. Use the manifest-bound initialization operation: run its `--dry-run` form, review every target and upstream readiness result, then run the operation without `--dry-run`. Product design uses `npm run init:product`; architecture design uses `npm run init:architecture` and requires the product strict Profile first.
6. Implement only the requested change. Never initialize a downstream stage to compensate for an unready upstream stage. Collect actual changed paths and resolve them again before verification.
7. Run every returned `commands` entry in order. Keep `FAIL`, `BLOCKED`, and `NOT_RUN` distinct from `PASS`.
8. Read `handoff.requiredFields` and `handoff.validationStates` from the manifest and use them for the final report.
